...
- [x] The card shows the platform's spend, reported revenue and reported ROAS
      alongside ours
- [x] **Every platform figure is labelled with its source.** A Reported Figure
      must never be mistakable for one of ours (ADR-0005)
- [x] Our Lookback Window is shown beside our figure and the platform's window
      beside theirs, so the merchant can see why they differ
- [x] **Reported Figures are never merged into our figures**, and never replace
...
- [x] **Reported Figures never feed Contribution Margin**, which is computed only
      from Orders whose goods have cost prices. A platform's conversion value has
      no cost basis behind it
- [x] Where the ad account's currency differs from the Store's, the figure is
      displayed in its own currency and the mismatch is explained
- [x] **No ROAS and no Contribution Margin is computed across a currency
      mismatch.** The merchant is shown nothing rather than something wrong
- [x] The additions do not change any of our own figures on the page
- [x] The card stays readable for an Ad with no Reported Figures at all, which is
      every Ad on a platform no sync covers
- [x] Covered end to end: both figures come back distinguishable from one read,
