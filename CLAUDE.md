...
- **Measurement travels with attribution**: the ad platform's browser identifiers (`_fbp`, and `_fbc` derived from `fbclid` at landing) and the visitor's consent answer ride on the same cart declaration and are frozen onto the order by the same copy. The storefront asks the API which pixel to load — connecting an ad platform switches measurement on with no storefront deploy — and a Store-level consent switch (off by default) holds back the pixel, the tracking script and the identifiers until a visitor accepts.
