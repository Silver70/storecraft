...
function ProductDetail({ product }: { product: Product }) {
    ...
    const useStructuredOptions = product.options.length > 0 && product.variants.some(v => v.optionValues.length > 0);

    const [selection, setSelection] = React.useState<OptionSelection>(() => initialSelection(product));
    const [variantId, setVariantId] = React.useState<string | null>(() => activeVariants[0]?.id ?? null);
                ...
                <div className="space-y-6">
                    <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-3">
                            <span className="text-xl font-medium">{formatMoney(displayPrice)}</span>
                    ...
                    {useStructuredOptions ? (
                        <VariantPicker product={product} selection={selection} onSelect={handleSelect} />
                    ) : (
                        <VariantSelect variants={product.variants} selectedId={variantId} onSelect={setVariantId} />
                    )}
                    ...
                    <AddToCartButton
                        ...
                        justAdded={justAdded}
                        error={addToCart.isError ? "We couldn't add that to your cart. Please try again." : null}
                        onAdd={handleAdd}
                    />
